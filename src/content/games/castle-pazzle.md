---
title: CASTLE PAZZLE
catch: 3つの城を記憶して、お題どおりに組み立てるタイムアタック。
category: unityroom
unityroomNo: 12
genre: パズル
platforms: [unityroom]
releaseDate: 2026-08-02
color: "#c8922e"
cover: /images/games/castle-pazzle/cover.jpg
links:
  - label: unityroomで遊ぶ
    url: https://unityroom.com/games/castle-pazzle
featured: true
dialogue:
  - speaker: onaga
    text: "CASTLE PAZZLEって、どんな城を作るゲームなん?"
  - speaker: akao
    text: "最初に見せられる3つの城を記憶して、バラバラになったパーツをお題どおりに組み立てるタイムアタックだよ。"
  - speaker: onaga
    text: "見本はずっと見られへんの?"
  - speaker: akao
    text: "組み立てが始まったら記憶が頼りだよ。パーツを回転させたり、カメラを動かしたりして正しい向きと場所を探すんだ。"
  - speaker: onaga
    text: "急いで建てるだけやなくて、覚える速さも大事なんやね。"
  - speaker: akao
    text: "そう。3つの城を完成させるまでのタイムを競うんだ。ハードモードにも挑戦できるよ。"
---

## 特徴

- お題の3つの城を記憶して組み立てる3Dパズル
- バラバラの城パーツをドラッグして地面や城の上に配置
- パーツとカメラを回転させ、正しい向きと場所を見極める
- 通常・ハードそれぞれのオンラインタイムランキングを搭載

## 操作方法

- **パーツを配置**: パーツをドラッグして、地面や城の上に置く
- **パーツを回転**: Rキー / マウスホイール
- **パーツを逆回転**: Qキー
- **カメラを回転**: 右ドラッグ、何もない場所を左ドラッグ、または矢印キー

3つの城をお題どおりに完成させ、クリアタイムを競います。
